...
N_QUBITS = 8                      # nr. unghiuri pt. stratul cuantic
N_CLASSES = 10                    # MNIST are 10 clase
BATCH = 64
LR = 1e-3
EPOCHS_TEACHER = 60

# >>> selectează front-endul de intrare:
USE_CNN_FRONTEND = True  # True = CNN→QLayer, False = PCA→QLayer

# ----------------------------
# Data loading (MNIST + PCA)  -- varianta clasică cu PCA (vectori)
# ----------------------------
def load_data(n_components=N_QUBITS, samples_per_class=100):
    """
    Încarcă MNIST (train+test), eșantionează balanced `samples_per_class` per clasă,
    reduce cu PCA la n_components, scalează în [-π, π].
    Returnează X (N x n_components), y (N,).
    """
    ...
    # PCA și scalare pentru encoding unghiuri
...
# ----------------------------
# Data loading (MNIST imagini) -- pentru hibridul CNN
# ----------------------------
def load_data_cnn(samples_per_class=100):
    """
    Încărcă MNIST ca imagini (1×28×28), selectează balanced `samples_per_class` per clasă.
    Returnează X_imgs [N,1,28,28] în [0,1], y [N].
    """
    tfm = transforms.Compose([transforms.ToTensor()])
    train_ds = datasets.MNIST(root="data_mnist", train=True,  download=True, transform=tfm)
    test_ds  = datasets.MNIST(root="data_mnist", train=False, download=True, transform=tfm)
    ...
    for c in range(N_CLASSES):
        ...
        X_sel.append(X[chosen])
        y_sel.append(y[chosen])
    X_sel = torch.cat(X_sel, dim=0).unsqueeze(1)  # [N,1,28,28]
...
class HybridQConv(nn.Module):
    """
     Small CNN to projection to N_QUBITS (angles) to quantum layer to classical head
    """
    def __init__(self, layers=2):
        super().__init__()
        # 1) Frontend CNN (1×28×28)
        self.cnn = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1),  # 28×28 → 16×28×28
        ...
        self.to_angles = nn.Sequential(
            ...
            nn.Linear(32*7*7, N_QUBITS)             #  N_QUBITS
        )

        # 2) Strat cuantic PennyLane
        dev = make_device()
        qnode = qnode_def(dev)
        weight_shapes = {"weights": (layers, 3, N_QUBITS)}
        self.q_layer = TorchLayer(qnode, weight_shapes)
    ...
    def forward(self, x):
        ...
        angles = torch.tanh(angles) * math.pi       # map la [-π, π]

        outs = [self.q_layer(sample) for sample in angles]  #  [N_QUBITS]
        q_out = torch.stack(outs)                            # [B,N_QUBITS]
...
def get_layer_groups_for_freeze(model):
    """
    Returns an ordered list of layer groups for CF-k,
...
def method_gradient_ascent(model, forget_loader, epochs=10, lr=LR, tag="GA",
    ...
    def _one_epoch(ep):
        ...
        for xb, yb in forget_loader:
            opt.zero_grad()
            loss = -criterion(model(xb), yb)  # ascent pe forget
            loss.backward()
            opt.step()
...
def method_eu_k(model, retain_loader, k=1, epochs=10, lr=LR, tag="EUk",
                eval_loader=None, patience=6, min_delta=0.0):
    """
    Reinitializes the last k *classical* layers (from the output side) and retrains them on Dr.
    For both models, we target the classical head: fc2 first, then fc1.
    """

    model = copy.deepcopy(model)
    # Reinit tail layers (capul clasic)
    if k >= 1:
...
def make_label_complement_loader(forget_loader, n_classes=N_CLASSES, batch=BATCH, shuffle=True):
    ...
    # Replicăm X pe dimensiunea 0 (N*(C-1), ...)
    reps = n_classes - 1
